...
public:
    ...
    void Compute(OpKernelContext *context) override {
        ...
        for (int t=1; t<T; ++t) {
            auto start = std::max(0, L-2*(T-t));
            auto end = std::min(2*t+2, L);
            ...
            for (int s=start; s<L; ++s) {
                ...
                if (s % 2==0) {
                    ...
                    alphas[s][t] = (alphas[s][t - 1] + alphas[s - 1][t - 1] + alphas[s - 2][t - 1])*logits(seq(l), t);
                }
...
//                auto t_vec = logits(t);
                vector<float> vec(n_vocab);
                for (int i = 0; i < n_vocab; ++i)
                    vec[i] = logits(t, i);
                auto target = vec[tmp];
                float v{0};
                for (auto e: vec)
                    v += std::max(0, e - target);
